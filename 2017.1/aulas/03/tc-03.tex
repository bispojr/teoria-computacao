...
%------------------------------------------
	\begin{frame}{Bônus (0,5 pt)}
		\begin{block}{Desafio}
			\begin{itemize}
				...
				\item Apresentação e resposta por escrito $\rightarrow$ \\segunda (15 de maio, 17h10); 
	...
	\begin{frame}{Configuração de uma MT}
		A configuração $C_1$ {\bf origina} a configuração $C_2$, se a máquina de Turing puder legitimamente ir de $C_1$ para $C_2$. \pause
		\begin{block}{Mais formalmente...}					Para:
			\begin{itemize}
				\item {\tt a, b, c} $\in \Gamma$, \pause
				\item {\tt u, v} $\in \Gamma^*$,  \pause
				\item os estados $q_i$ e $q_j$, \pause
				\item as configurações {\tt ua}$q_i${\tt bv} e {\tt u}$q_j${\tt acv}. \pause
				\end{itemize}
			\pause	
			Digamos que 
			\begin{center}
